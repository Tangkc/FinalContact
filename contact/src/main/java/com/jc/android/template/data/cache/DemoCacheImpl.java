 ...
 */
package com.jc.android.template.data.cache;
...
public class DemoCacheImpl implements DemoCache {
	...
	/**
	 * Constructor of the class {@link DemoCacheImpl}.
	 * <p/>
	 * <p/>
	 ...
	 */
	public DemoCacheImpl(Context applicationContext) {
	 ...
	 */
	public DemoCacheImpl(Context context, JsonSerializer userCacheSerializer,
	...
	@Override
	public synchronized Observable<DemoEntity> get(final int userId) {
		return Observable.create(new Observable.OnSubscribe<DemoEntity>() {
			@Override
			public void call(Subscriber<? super DemoEntity> subscriber) {
				File userEntityFile = DemoCacheImpl.this.buildFile(userId);
				String fileContent = DemoCacheImpl.this.fileManager.readFileContent(userEntityFile);
				DemoEntity demoEntity = DemoCacheImpl.this.serializer.deserialize(fileContent,DemoEntity.class);
	...
	@Override
	public synchronized void put(DemoEntity demoEntity) {
		if (demoEntity != null) {
			File userEntitiyFile = this.buildFile(demoEntity.getUserId());
			if (!isCached(demoEntity.getUserId())) {
				String jsonString = this.serializer.serialize(demoEntity);
